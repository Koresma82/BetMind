...
exports.handler = async (event) => {
  ...
  try {
    const { date, prediction } = JSON.parse(event.body || '{}');
    if (!date || !prediction) return json(400, { error: 'date e prediction obrigatórios' });
    await setDoc(`users/${payload.sub}/predictions/${date}`, { data: date, savedAt: new Date().toISOString(), resumo_dia: prediction.resumo_dia || '', melhor_aposta_dia: prediction.melhor_aposta_dia || '', jogos: prediction.jogos || [] });
